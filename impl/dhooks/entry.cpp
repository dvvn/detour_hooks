...
basic_hook_entry::~basic_hook_entry() = default;

struct hook_entry_impl : basic_hook_entry
{
    hook_entry_impl();
    ~hook_entry_impl() override;

    hook_entry_impl(hook_entry_impl&& other);
    hook_entry_impl& operator=(hook_entry_impl&& other);
    ...
    void* get_replace_method() const override;

    void set_target_method(void* getter) override;
    void set_replace_method(void* getter) override;
...
{
    return std::make_unique<hook_entry_impl>();
}
...
hook_entry_impl::hook_entry_impl() = default;

hook_entry_impl::~hook_entry_impl()
{
    disable();
}

hook_entry_impl::hook_entry_impl(hook_entry_impl&& other) = default;
hook_entry_impl& hook_entry_impl::operator=(hook_entry_impl&& other) = default;
...
bool hook_entry_impl::created() const
{
    return !trampoline_.empty();
}

bool hook_entry_impl::enabled() const
{
    return enabled_;
}

class prepared_memory_block : public nstd::mem::block
{
    nstd::mem::protect protect_;
  ...
  public:
    ...
    prepared_memory_block(void* target, bool patch_above)
    {
        ...
        *static_cast<nstd::mem::block*>(this) = {target_ptr, target_ptr_size};
        ...
        if (!this->have_flags(PAGE_EXECUTE_READWRITE))
        {
            ...
            runtime_assert(protect_.has_value());
        }
    }
};

bool hook_entry_impl::enable()
{
    if (enabled_)
        return false;
...
void* hook_entry_impl::get_original_method() const
{
...
void* hook_entry_impl::get_target_method() const
{
    return target_;
}

void* hook_entry_impl::get_replace_method() const
{
    return detour_;
}

void hook_entry_impl::set_target_method(void* getter)
{
...
void hook_entry_impl::set_replace_method(void* getter)
{
